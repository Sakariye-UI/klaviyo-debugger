...
function buildCampaignSteps(scenario, fields, timeRange) {
  ...
  const scenarios = {
    ...
    skipped: {
      ...
      foundMsg: `<strong>Campaign skip log found</strong><br>
        <br>Expand the raw logs to see the specific "Unable to send" reason. Common reasons include:<br>
        <ul>
          <li><strong>Invalid email</strong> — Profile has a malformed or invalid email address</li>
          <li><strong>Profile is suppressed</strong> — Profile was globally suppressed (bounce, spam, manual)</li>
          <li><strong>Profile opted out</strong> — Profile unsubscribed from the sending list</li>
          <li><strong>Outside send time</strong> — Campaign had a send time restriction and the profile's timezone was outside it</li>
          <li><strong>Duplicate profile</strong> — Another profile with the same email was also in the send</li>
        </ul>`
    },
