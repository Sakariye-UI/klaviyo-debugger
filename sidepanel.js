...
const PERSIST_KEY    = 'kdbg-form-state';
const PERSIST_TTL_MS = 24 * 60 * 60 * 1000;
const FIELD_IDS = [
  'f_accountId','f_flowId','f_profileId','f_activityId','f_flowMessageId','f_listSegId',
  'c_accountId','c_campaignId','c_profileId',
  'l_accountId','l_profileId','l_listId','l_segmentId',
  'b_accountId',
  'fm_accountId','fm_formId',
  'p_accountId','p_profileId',
  'r_accountId',
  'i_accountId','i_integrationType',
  'timeManualValue','timeDateFrom','timeDateTo'
];
...
function saveFormState() {
  ...
  localStorage.setItem(PERSIST_KEY, JSON.stringify({
    savedAt: Date.now(),
    selectedArea, selectedTrigger,
    selectedCampaignScenario, selectedListScenario, selectedBillingScenario,
    selectedFormScenario, selectedReviewScenario,
    selectedSuppressionChecks, selectedTimeMode, selectedTimeMinutes,
    fields
  }));
}
...
function restoreFormState() {
  let state;
  try { state = JSON.parse(localStorage.getItem(PERSIST_KEY) || 'null'); } catch(e) { return; }
  if (!state || !state.savedAt || (Date.now() - state.savedAt) > PERSIST_TTL_MS) return;

  // Restore area button
  if (state.selectedArea) {
    const btn = document.querySelector(`.area-btn[data-area="${state.selectedArea}"]`);
    if (btn) btn.click();
  }
  // Restore trigger pill
  if (state.selectedTrigger) {
    const pill = document.querySelector(`#triggerType .pill[data-value="${state.selectedTrigger}"]`);
    if (pill) pill.click();
  }
  // Restore scenario pills (clicks re-fire conditional UI logic)
  const pillGroups = {
    campaignScenario: state.selectedCampaignScenario,
    listScenario:     state.selectedListScenario,
    billingScenario:  state.selectedBillingScenario,
    formScenario:     state.selectedFormScenario,
    reviewScenario:   state.selectedReviewScenario
  };
  Object.entries(pillGroups).forEach(([groupId, value]) => {
    if (value) {
      const pill = document.querySelector(`#${groupId} .pill[data-value="${value}"]`);
      if (pill) pill.click();
    }
  });
  // Restore suppression checkboxes
  if (state.selectedSuppressionChecks) {
    selectedSuppressionChecks = state.selectedSuppressionChecks;
    document.querySelectorAll('.supp-check').forEach(chk => {
      chk.checked = selectedSuppressionChecks.includes(chk.dataset.check);
    });
  ...
  // Restore time mode + range
  if (state.selectedTimeMode === 'date') {
    applyTimeMode('date');
  } else if (state.selectedTimeMinutes) {
    setTimeMinutes(state.selectedTimeMinutes, 'restore');
  }
}

// Save state whenever any field input changes
document.addEventListener('input', e => {
  if (e.target.classList.contains('field-input')) saveFormState();
});
...
function setTimeMinutes(minutes, source) {
  ...
  if (source !== 'restore') saveFormState();
}
...
function buildStepCard(num, label, description, tool, idx) {
  ...
  return { el, body: el.querySelector('.step-body'), num, tool };
}
...
function handleBackgroundMessage(msg) {
  ...
  if (msg.type === 'STEP_ERROR') {
    ...
    const toolName = card.tool === 'splunk' ? 'Splunk' : 'Chronosphere';
    const toolUrl  = card.tool === 'splunk' ? 'klaviyo.splunkcloud.com' : 'klaviyo.chronosphere.io';
    card.body.innerHTML = `
      <div class="step-verdict">
        🔐 <strong>${toolName} isn't accessible</strong> — make sure you're logged into
        <code>${toolUrl}</code> in another tab, then retry.
        <br><br>
        <button class="step-action retry-step-btn">Retry this step →</button>
      </div>`;
    card.body.querySelector('.retry-step-btn').addEventListener('click', () => handleContinueFrom(msg.stepIndex));
    card.body.classList.add('open');
  }
